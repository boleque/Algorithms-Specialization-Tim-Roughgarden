...
def r_select(arr, order_statistic):
    return _r_select(A, 0, len(A) - 1, order_statistic)

def _r_select(arr, left_idx, right_idx, order_statistic):
    if left_idx == right_idx:
        return arr[left_idx]
    elif left_idx < right_idx:
        partition_idx = _partition(arr, left_idx, right_idx)
        k = partition_idx - left_idx + 1 # !!!
        if k == order_statistic:
            return arr[partition_idx]
        elif k > order_statistic:
            return _r_select(arr, left_idx, partition_idx - 1, order_statistic)
        else:
            return _r_select(arr, partition_idx + 1, right_idx, order_statistic - k)
